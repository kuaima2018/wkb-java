...
public class UserController extends BaseController {
    ...
    @ResponseBody
    public WktStatusResult check(@RequestBody JsonUserPass jsonUserPass)
    {
        ...
        try{
            //wkt-验证码校验
            WkbUser wkbUser=wkbUserService.queryUserByIdentifier(jsonUserPass.getMobile());
            if(wkbUser==null||wkbUser.getuId()==null){

            }else {
                wktStatus.setErrorCode(100);
                wktStatus.setErrorMessage("用户已存在");
            }
        ...
        }catch (Exception exp)
        {
            ...
            logger.error("check user error:", exp);
        }
        return new WktStatusResult(wktStatus);
    }
    ...
    public WktStatusResult register(@RequestBody JsonUserPass jsonUserPass)
    {
        ...
        try{
            ...
            if(!wkbBizResult.isSucc())
            {
                ...
                //im add
                try {
                    imService.registerUser(wkbUser.getuId().toString(), wkbUser.getuName(), wkbUser.getuPwd());
                }catch (Exception expIm){
                    wkbUserService.removeUser(wkbUser.getuId());
                    throw expIm;
                }
            }
    ...
    public WktResult updateUser(@RequestBody JsonUserUpdate jsonUserUpdate)
    {
        ...
        try
        {
            ...
            if(StringUtils.isNotBlank(jsonUserUpdate.getImage())&&StringUtils.isNotBlank(jsonUserUpdate.getImageName()))
            {
                ...
                imageName=String.valueOf(new Date().getTime())+"-"+wkbUser.getuId().toString();
                if(index>0)
                {
            ...
            //
            String userName="";
            if(StringUtils.isNotBlank(jsonUserUpdate.getNickName())){
                if(!jsonUserUpdate.getNickName().equals(wkbUser.getuName())){
                    //im update
                    userName=jsonUserUpdate.getNickName();
                }
            }
            ...
            else
            {
                ...
                if(StringUtils.isNotBlank(userName)){
                    imService.resetUser(wkbUser.getuId().toString(),userName,null);
                }
                //
    ...
    public WktStatusResult resetPassword(@RequestBody JsonUserPass jsonUserPass)
    {
        ...
        try
        {
            ...
            if(!wkbBizResult.isSucc())
            {
                ...
                //im update
                imService.resetUser(wkbUser.getuId().toString(),null,jsonUserPass.getPassword());
            }
        }catch (WkbBizException wkbExp)
        {
    ...
    public WktStatusResult applyMembership(@RequestBody JsonUserApplyQuery jsonUserApplyQuery) {
        ...
        try {
            ...
            wkbFriendapply.setRemark(jsonUserApplyQuery.getRemark());
    ...
    public WktResult queryFriendApply(@RequestBody JsonTaskQuery jsonTaskQuery)
    {
        ...
        try{
            List<WkbFriendapply> wkbFriendapplyList = wkbFriendapplyService.queryApplyList(wkbFriendapplyQuery, true, true, null, index, pagesize);
                ...
                if(userIdList.size()>0)
                {
                    ...
                    for(JsonFriendApply jsonFriendApply:jsonFriendApplyList)
                    {
                        for(WkbUser wkbUser:wkbUserList)
                        {
                            ...
                            {
                                jsonFriendApply.setTitle(wkbUser.getuTitle());
                                jsonFriendApply.setCompany(wkbUser.getuCompany());
    ...
    public WktStatusResult deleteFriendApply(@RequestBody JsonUserApplyQuery jsonUserApplyQuery)
    {
        WktStatus wktStatus=new WktStatus();
        if(jsonUserApplyQuery==null||jsonUserApplyQuery.getUserId()==null||(jsonUserApplyQuery.getRequestIds()==null
                && StringUtils.isBlank(jsonUserApplyQuery.getStrRequestIds())))
        {
            wktStatus.setErrorCode(1);
            wktStatus.setErrorMessage("未提供参数");
            return new WktStatusResult(wktStatus);
        ...
        try{
            List<Integer> idList=jsonUserApplyQuery.getRequestIds();
            if(StringUtils.isNotBlank(jsonUserApplyQuery.getStrRequestIds()))
                idList=ListHelper.parseStr(jsonUserApplyQuery.getStrRequestIds());
            WkbBizResult wkbBizResult=wkbFriendapplyService.removeApplyByIds(jsonUserApplyQuery.getUserId(),idList);
            if(!wkbBizResult.isSucc())
            {
                wktStatus.setErrorCode(-101);
                wktStatus.setErrorMessage(this.getBizError(wkbBizResult.getCode()));
            ...
            if(wkbUser!=null)
            {
                if(StringUtils.isNotBlank(wkbUser.getuAppsecret()))
                {
                    ...
                    else
                    {
                        //设置返回信息
                        JsonUser jsonUser=new JsonUser();
                        WktUserUtil.convert(clerkUser,jsonUser);
                        if(StringUtils.isNotBlank(clerkUser.getImageUrl()))
                            jsonUser.setImageUrl(systemConfigure.getImageServer()+clerkUser.getImageUrl());
                        wktResult.setResult(jsonUser);
                    }
                }
            }
    ...
    @ResponseBody
    public WktResult getFriends(@RequestBody JsonUserQuery jsonUserQuery)
    {
        WktResult wktResult=new WktResult();
        if(jsonUserQuery==null)
        {
            wktResult.getStatus().setErrorCode(1);
            wktResult.getStatus().setErrorMessage("未提供参数");
            return wktResult;
        ...
        int startPos=(jsonUserQuery.getIndex()-1)*jsonUserQuery.getPageSize();
        int endPos=jsonUserQuery.getPageSize();
        List<WkbUser> wkbUserList=wkbUserService.queryFriends(jsonUserQuery.getUserId(),startPos,endPos);
        if(wkbUserList!=null&&wkbUserList.size()>0)
        {
            List<JsonUser> jsonUserList=new ArrayList<JsonUser>();
            for(WkbUser wkbUser:wkbUserList)
    ...
    @ResponseBody
    public WktStatusResult delFriends(@RequestBody JsonUserApplyQuery jsonUserApplyQuery)
    {
        WktStatus wktStatus=new WktStatus();
        if(jsonUserApplyQuery==null||jsonUserApplyQuery.getUserId()==null
                ||jsonUserApplyQuery.getTargetId()==null)
        {
            wktStatus.setErrorCode(1);
            wktStatus.setErrorMessage("未提供参数");
            return new WktStatusResult(wktStatus);
        ...
        try{
            wkbFriendsService.removeFriend(jsonUserApplyQuery.getUserId(),jsonUserApplyQuery.getTargetId());
            wkbFriendsService.removeFriend(jsonUserApplyQuery.getTargetId(),jsonUserApplyQuery.getUserId());
        }
        catch (Exception exp)
        {
            logger.error("delFriends unknown error:",exp);
            wktStatus.setErrorCode(-100);
            wktStatus.setErrorMessage(exp.getMessage());
        }
